<#
.SYNOPSIS
  Builds releases\GDDraw-Plus-v<version>.zip: addons/GDDraw plus the license files, with forward-slash entry names.
...
$config = [System.IO.File]::ReadAllText((Join-Path $root "addons\GDDraw\plugin.cfg"))
$version = [regex]::Match($config, 'version="([^"]+)"').Groups[1].Value
if (-not $version) { throw "No version in addons\GDDraw\plugin.cfg" }
$releases = Join-Path $root "releases"
[System.IO.Directory]::CreateDirectory($releases) | Out-Null
$zipPath = Join-Path $releases "GDDraw-Plus-v$version.zip"
if (Test-Path $zipPath) { [System.IO.File]::Delete($zipPath) }
Add-Type -AssemblyName System.IO.Compression
Add-Type -AssemblyName System.IO.Compression.FileSystem
$zip = [System.IO.Compression.ZipFile]::Open($zipPath, [System.IO.Compression.ZipArchiveMode]::Create)
try {
    $files = @(Get-ChildItem (Join-Path $root "addons\GDDraw") -Recurse -File | Where-Object { $_.Extension -ne ".import" })
